...
public class MyGlobalFilter implements GlobalFilter, Ordered {
    ...
    public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
        ...
        log.info("请求接口为：{}", path);
        if (path.contains("/user/send") || path.contains("/user/verify") || path.contains("/note/v2/api-docs") || path.contains("/note/getNoteList") || path.contains("/user/uploads") || path.contains("/search")) {
            log.info("请求接口放行");
